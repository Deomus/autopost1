...
async def download_video(url: str, output_path: str, proxy_url: str = None, headers=None):
    """Скачивает видео по URL и сохраняет его в файл."""
    try:
...
async def infinity_scrolling(id: str):
    ...
    # Инициализация браузера и контекста с использованием контекстного менеджера
    async with async_playwright() as playwright:
        ...
        while True:
            ...
            try:
                ...
                # Переходим на страницу с рилсами
                await page.goto("https://www.instagram.com/reels/")
                await asyncio.sleep(5)
                # await page.screenshot(path=f"debug_{uuid.uuid4()}.png", full_page=True)

                # Эмулируем нажатие Enter для активации скроллинга
                ...
                while queue_len < 15:
                    ...
                    if postcards := parser.css(postcard):
                        ...
                        for card in postcards:
                            # logger.info(f"кол-во постов {card.text()}")
                                ...
                                if like > user.likes:
                                    ...
                                    if await r.sadd(id, video_url):
                                        ...
                                        try:
                                            ...
                                            logger.error(f"ID: {id} ОШИБКА ЗАГРУЗКИ В БД или скачивание файла: {e}")
                                            continue
                                        await asyncio.sleep(5)
                            else:
                                logger.warning("нет ссылки на видео")

                        # Эмулируем скроллинг вниз
                ...
                # Если очередь заполнена, ждем 5 минут
                ...
                # Закрываем страницу после каждой итерации
...
async def infinity_posting(id: str):
    ...
    while True:
        try:
            async with async_playwright() as playwright:
                # browser = await playwright.chromium.connect("ws://0.0.0.0:3030/")
